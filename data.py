...
CHECKLIST_DATA = {
    "Linux Ubuntu 24.04": [
        {"danh_muc": "1. Cấu hình cơ bản", "items": [
            {"stt": "1", "muc": "Kiểm tra hostname", "cmd": "hostname -f", "row_idx": 5},
            {"stt": "2", "muc": "Kiểm tra IP tĩnh", "cmd": "ip -br a", "row_idx": 6},
            {"stt": "3", "muc": "Default Gateway và DNS được cấu hình chính xác.", "cmd": "ip route; nslookup google.com", "row_idx": 7},
            {"stt": "4", "muc": "Hostname đã được khai báo trong file /etc/hosts.", "cmd": "cat /etc/hosts", "row_idx": 8},
            {"stt": "5", "muc": "Hệ điều hành đã được cập nhật đầy đủ các package.", "cmd": "apt list --upgradable", "row_idx": 9},
        ]},
        {"danh_muc": "2. SSH Hardening", "items": [
            ...
            {"stt": "12", "muc": "Chỉ sử dụng các Cipher và Key Exchange (KEX) Algorithm mạnh", "cmd": "grep -E 'KexAlgorithms|Ciphers|MACs' /etc/ssh/sshd_config", "row_idx": 17},
            {"stt": "13", "muc": "Cấu hình SSH đã được kiểm tra cú pháp (syntax check)", "cmd": "sudo sshd -t", "row_idx": 18},
        ]},
        {"danh_muc": "3. Password Policy", "items": [
            {"stt": "14", "muc": "minlen = 14 trong pwquality.conf", "cmd": "grep minlen /etc/security/pwquality.conf", "row_idx": 20},
            {"stt": "15", "muc": "Chính sách mật khẩu yêu cầu ký tự hoa, thường, số, đặc biệt.", "cmd": "grep -E 'dcredit|ucredit|ocredit|lcredit' /etc/security/pwquality.conf", "row_idx": 21},
            {"stt": "16", "muc": "Lưu lịch sử 5 mật khẩu gần nhất bằng pam_pwhistory", "cmd": "grep pam_pwhistory /etc/pam.d/common-password", "row_idx": 22},
            {"stt": "17", "muc": "Mật khẩu được lưu trữ bằng thuật toán băm SHA-512.", "cmd": "grep sha512 /etc/pam.d/common-password", "row_idx": 23},
        ]},
        {"danh_muc": "4. Xác thực RADIUS", "items": [
            {"stt": "18", "muc": "Máy chủ RADIUS được cấu hình chính xác", "cmd": "cat /etc/pam_radius_auth.conf", "row_idx": 25},
            {"stt": "19", "muc": "PAM common-auth được cấu hình theo đúng thứ tự xác thực", "cmd": "cat /etc/pam.d/common-auth", "row_idx": 26},
            {"stt": "20", "muc": "Tham số KbdInteractiveAuthentication = yes", "cmd": "grep KbdInteractiveAuthentication /etc/ssh/sshd_config", "row_idx": 27},
            {"stt": "21", "muc": "Kiểm tra đăng nhập bằng tài khoản RADIUS thành công.", "cmd": "Thử SSH bang user RADIUS", "row_idx": 28},
        ]},
        {"danh_muc": "5. Firewall (Iptables)", "items": [
            {"stt": "22", "muc": "iptables đang hoạt động với các rule được cấu hình đúng", "cmd": "sudo iptables -L -n -v", "row_idx": 30},
            {"stt": "23", "muc": "INPUT default policy là DROP/REJECT", "cmd": "sudo iptables -L INPUT -n | head -5", "row_idx": 31},
            {"stt": "24", "muc": "FORWARD default DROP", "cmd": "sudo iptables -L FORWARD -n | head -3", "row_idx": 32},
            {"stt": "25", "muc": "SSH chỉ được phép từ VPN range và system terminal", "cmd": "sudo iptables -L INPUT -n | grep 10.31.100.18", "row_idx": 33},
            {"stt": "26", "muc": "Port 9100 chỉ được phép từ Prometheus Server", "cmd": "sudo iptables -L INPUT -n | grep 9100", "row_idx": 34},
            {"stt": "27", "muc": "iptables-persistent hoạt động, rule được giữ sau reboot", "cmd": "sudo systemctl status netfilter-persistent", "row_idx": 35},
        ]},
        {"danh_muc": "6. Fail2ban", "items": [
            {"stt": "28", "muc": "Dịch vụ fail2ban đang chạy và hoạt động bình thường.", "cmd": "sudo systemctl status fail2ban", "row_idx": 37},
            {"stt": "29", "muc": "Jail SSH được kích hoạt (bantime=3600, maxretry=4).", "cmd": "sudo fail2ban-client status sshd", "row_idx": 38},
            {"stt": "30", "muc": "Danh sách ignoreip đã bao gồm IP system terminal", "cmd": "sudo fail2ban-client get sshd ignoreip", "row_idx": 39},
        ]},
        {"danh_muc": "7. NTP", "items": [
            {"stt": "31", "muc": "NTP Server được cấu hình là ntp.vngdc.local", "cmd": "cat /etc/systemd/timesyncd.conf", "row_idx": 41},
            {"stt": "32", "muc": "NTP đang đồng bộ thành công", "cmd": "timedatectl timesync-status", "row_idx": 42},
            {"stt": "33", "muc": "Timezone được cấu hình là Asia/Ho_Chi_Minh", "cmd": "timedatectl | grep zone", "row_idx": 43},
        ]},
        {"danh_muc": "8. Logging & Monitoring", "items": [
            {"stt": "34", "muc": "rsyslog được cấu hình để forward log về Syslog Server", "cmd": "grep syslog.vngdc.local /etc/rsyslog.conf", "row_idx": 45},
            {"stt": "35", "muc": "Thư mục lưu log của máy chủ tồn tại trên Syslog Server", "cmd": "Kiểm tra trên syslog server 10.31.100.20", "row_idx": 46},
            {"stt": "36", "muc": "Wazuh agent đang hoạt động", "cmd": "systemctl status wazuh-agent", "row_idx": 47},
            {"stt": "37", "muc": "node_exporter đang hoạt động và phản hồi /metrics", "cmd": "curl http://localhost:9100/metrics | head -5", "row_idx": 48},
            {"stt": "38", "muc": "Target hiển thị trạng thái Healthy/UP trên Prometheus", "cmd": "Kiểm tra trên Prometheus UI", "row_idx": 49},
        ]},
        {"danh_muc": "9. Dịch vụ không cần thiết", "items": [
            {"stt": "39", "muc": "ModemManager disabled", "cmd": "systemctl is-enabled ModemManager", "row_idx": 51},
            {"stt": "40", "muc": "thermald disabled", "cmd": "systemctl is-enabled thermald", "row_idx": 52},
            {"stt": "41", "muc": "apport disabled", "cmd": "systemctl is-enabled apport", "row_idx": 53},
        ]},
        {"danh_muc": "10. Crontab va Scripts", "items": [
            {"stt": "42", "muc": "Script backup_iptables.sh tồn tại", "cmd": "ls -la /opt/dc_script/backup_iptables.sh", "row_idx": 55},
            {"stt": "43", "muc": "Script free_mem.sh tồn tại", "cmd": "ls -la /opt/dc_script/free_mem.sh", "row_idx": 56},
            {"stt": "44", "muc": "Crontab entries có (netfilter-persist, free_mem, clear audit)", "cmd": "sudo cat /etc/crontab", "row_idx": 57},
            {"stt": "45", "muc": "Iptables.save tồn tại và áp dụng các rule", "cmd": "sudo iptables.save", "row_idx": 58},
        ]}
    ],
...
AUDIT_DATA = {
    "Linux Ubuntu 24.04": [
        ...
        {"danh_muc": "2. Password Policy", "items": [
            {"stt": "8", "nguy_co": "Cao", "muc": "grep minlen /etc/security/pwquality.conf", "expected": "14", "row_idx": 13},
            {"stt": "9", "nguy_co": "Cao", "muc": "Yêu cầu độ phức tạp mật khẩu", "expected": "-1 cho mỗi loại", "row_idx": 14},
            {"stt": "10", "nguy_co": "Trung bình", "muc": "grep pam_pwhistory /etc/pam.d/common-password", "expected": "remember=5", "row_idx": 15},
            {"stt": "11", "nguy_co": "Cao", "muc": "grep sha512 /etc/pam.d/common-password", "expected": "sha512", "row_idx": 16},
        ]},
        {"danh_muc": "3. Xác thực RADIUS", "items": [
            {"stt": "12", "nguy_co": "Cao", "muc": "cat /etc/pam_radius_auth.conf", "expected": "Cấu hình IP Radius", "row_idx": 18},
            {"stt": "13", "nguy_co": "Cao", "muc": "grep KbdInteractiveAuthentication /etc/ssh/sshd_config", "expected": "yes", "row_idx": 19},
            {"stt": "14", "nguy_co": "Cao", "muc": "cat /etc/pam.d/common-auth", "expected": "RADIUS trước, local fallback", "row_idx": 20},
            {"stt": "15", "nguy_co": "Cao", "muc": "Thử SSH bang user RADIUS", "expected": "Thành công", "row_idx": 21},
        ]},
        {"danh_muc": "4. Firewall", "items": [
            {"stt": "16", "nguy_co": "Cao", "muc": "sudo iptables -L -n -v", "expected": "Hoạt động và có rule", "row_idx": 23},
            {"stt": "17", "nguy_co": "Cao", "muc": "sudo iptables -L INPUT -n", "expected": "policy DROP/REJECT", "row_idx": 24},
            {"stt": "18", "nguy_co": "Cao", "muc": "sudo iptables -L FORWARD -n", "expected": "policy DROP", "row_idx": 25},
            {"stt": "19", "nguy_co": "Cao", "muc": "sudo iptables -L INPUT -n | grep 22", "expected": "Whitelist IP rõ ràng", "row_idx": 26},
            {"stt": "20", "nguy_co": "Cao", "muc": "systemctl is-enabled netfilter-persistent", "expected": "enabled", "row_idx": 27},
        ]},
        {"danh_muc": "5. Fail2ban", "items": [
            {"stt": "21", "nguy_co": "Cao", "muc": "systemctl is-active fail2ban", "expected": "active", "row_idx": 29},
            {"stt": "22", "nguy_co": "Cao", "muc": "fail2ban-client status sshd", "expected": "bantime=3600, maxretry=4", "row_idx": 30},
        ]},
        {"danh_muc": "6. NTP", "items": [
            {"stt": "23", "nguy_co": "Trung bình", "muc": "timedatectl timesync-status", "expected": "ntp.vngdc.local", "row_idx": 32},
            {"stt": "24", "nguy_co": "Thấp", "muc": "timedatectl | grep zone", "expected": "Asia/Ho_Chi_Minh", "row_idx": 33},
        ]},
        {"danh_muc": "7. Logging", "items": [
            {"stt": "25", "nguy_co": "Cao", "muc": "grep '@' /etc/rsyslog.conf", "expected": "Gửi log về Server", "row_idx": 35},
            {"stt": "26", "nguy_co": "Cao", "muc": "systemctl is-active wazuh-agent", "expected": "active", "row_idx": 36},
            {"stt": "27", "nguy_co": "Trung bình", "muc": "systemctl is-active node_exporter", "expected": "active", "row_idx": 37},
            {"stt": "28", "nguy_co": "Trung bình", "muc": "Kiểm tra Prometheus UI", "expected": "UP", "row_idx": 38},
        ]},
        {"danh_muc": "8. Dịch vụ không cần thiết", "items": [
            {"stt": "29", "nguy_co": "Trung bình", "muc": "systemctl is-enabled ModemManager", "expected": "disabled", "row_idx": 40},
            {"stt": "30", "nguy_co": "Thấp", "muc": "systemctl is-enabled thermald", "expected": "disabled", "row_idx": 41},
            {"stt": "31", "nguy_co": "Trung bình", "muc": "systemctl is-enabled apport", "expected": "disabled", "row_idx": 42},
        ]},
        {"danh_muc": "9. Scripts & Crontab", "items": [
            {"stt": "32", "nguy_co": "Trung bình", "muc": "ls /opt/dc_script/", "expected": "Đầy đủ file bash", "row_idx": 44},
            {"stt": "33", "nguy_co": "Trung bình", "muc": "cat /etc/crontab", "expected": "Có đủ 3 entry", "row_idx": 45},
        ]},
    ],
