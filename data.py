...
CHECKLIST_DATA = {
    "Linux Ubuntu 24.04": [
        ...
        {"danh_muc": "2. SSH Hardening", "items": [
            ...
            {"stt": "14", "muc": "PermitEmptyPasswords = no", "cmd": "grep PermitEmptyPasswords /etc/ssh/sshd_config", "row_idx": 19},
            {"stt": "15", "muc": "UsePAM = yes", "cmd": "grep 'UsePAM' /etc/ssh/sshd_config", "row_idx": 20},
            {"stt": "16", "muc": "GSSAPIAuthentication = no", "cmd": "grep GSSAPIAuthentication /etc/ssh/sshd_config", "row_idx": 21},
            {"stt": "17", "muc": "HostbasedAuthentication = no / IgnoreRhosts = yes", "cmd": "grep -E 'HostbasedAuthentication|IgnoreRhosts' /etc/ssh/sshd_config", "row_idx": 22},
            {"stt": "18", "muc": "LogLevel = VERBOSE", "cmd": "grep LogLevel /etc/ssh/sshd_config", "row_idx": 23},
            {"stt": "19", "muc": "MaxStartups = 10:30:60", "cmd": "grep MaxStartups /etc/ssh/sshd_config", "row_idx": 24},
            {"stt": "20", "muc": "ClientAliveInterval = 900", "cmd": "grep -E 'ClientAliveInterval|ClientAliveCountMax' /etc/ssh/sshd_config", "row_idx": 25},
        ]},
        {"danh_muc": "3. Password Policy", "items": [
            {"stt": "21", "muc": "minlen = 14 trong pwquality.conf", "cmd": "grep minlen /etc/security/pwquality.conf", "row_idx": 27},
            {"stt": "22", "muc": "Chính sách mật khẩu yêu cầu ký tự hoa, thường, số, đặc biệt.", "cmd": "grep -E 'dcredit|ucredit|ocredit|lcredit' /etc/security/pwquality.conf", "row_idx": 28},
            {"stt": "23", "muc": "Lưu lịch sử 5 mật khẩu gần nhất bằng pam_pwhistory", "cmd": "grep pam_pwhistory /etc/pam.d/common-password", "row_idx": 29},
            {"stt": "24", "muc": "Mật khẩu được lưu trữ bằng thuật toán băm SHA-512.", "cmd": "grep sha512 /etc/pam.d/common-password", "row_idx": 30},
            {"stt": "25", "muc": "pwquality.conf: difok=2, maxrepeat=3, maxsequence=3, dictcheck=1", "cmd": "grep -E 'difok|maxrepeat|maxsequence|dictcheck' /etc/security/pwquality.conf", "row_idx": 31},
            {"stt": "26", "muc": "faillock.conf: deny=5, unlock_time=900 (15 phút)", "cmd": "grep -E 'deny|unlock_time' /etc/security/faillock.conf", "row_idx": 32},
            {"stt": "27", "muc": "faillock.conf: even_deny_root", "cmd": "grep 'even_deny_root' /etc/security/faillock.conf", "row_idx": 33},
        ]},
        {"danh_muc": "4. Xác thực RADIUS", "items": [
            {"stt": "28", "muc": "Máy chủ RADIUS được cấu hình chính xác", "cmd": "cat /etc/pam_radius_auth.conf", "row_idx": 35},
            {"stt": "29", "muc": "PAM common-auth được cấu hình theo đúng thứ tự xác thực", "cmd": "cat /etc/pam.d/common-auth", "row_idx": 36},
            {"stt": "30", "muc": "Tham số KbdInteractiveAuthentication = yes", "cmd": "grep KbdInteractiveAuthentication /etc/ssh/sshd_config", "row_idx": 37},
            {"stt": "31", "muc": "Kiểm tra đăng nhập bằng tài khoản RADIUS thành công.", "cmd": "Thử SSH bang user RADIUS", "row_idx": 38},
        ]},
        {"danh_muc": "5. Firewall (Iptables)", "items": [
            {"stt": "32", "muc": "iptables đang hoạt động với các rule được cấu hình đúng", "cmd": "sudo iptables -L -n -v", "row_idx": 40},
            {"stt": "33", "muc": "INPUT default policy là DROP/REJECT", "cmd": "sudo iptables -L INPUT -n | head -5", "row_idx": 41},
            {"stt": "34", "muc": "FORWARD default DROP", "cmd": "sudo iptables -L FORWARD -n | head -3", "row_idx": 42},
            {"stt": "35", "muc": "SSH chỉ được phép từ VPN range và system terminal", "cmd": "sudo iptables -L INPUT -n | grep 10.31.100.18", "row_idx": 43},
            {"stt": "36", "muc": "Port 9100 chỉ được phép từ Prometheus Server", "cmd": "sudo iptables -L INPUT -n | grep 9100", "row_idx": 44},
            {"stt": "37", "muc": "iptables-persistent hoạt động, rule được giữ sau reboot", "cmd": "sudo systemctl status netfilter-persistent", "row_idx": 45},
        ]},
        {"danh_muc": "6. Fail2ban", "items": [
            {"stt": "38", "muc": "Dịch vụ fail2ban đang chạy và hoạt động bình thường.", "cmd": "sudo systemctl status fail2ban", "row_idx": 47},
            {"stt": "39", "muc": "Jail SSH được kích hoạt (bantime=3600, maxretry=4).", "cmd": "sudo fail2ban-client status sshd", "row_idx": 48},
            {"stt": "40", "muc": "Danh sách ignoreip đã bao gồm IP system terminal", "cmd": "sudo fail2ban-client get sshd ignoreip", "row_idx": 49},
        ]},
        {"danh_muc": "7. NTP", "items": [
            {"stt": "41", "muc": "NTP Server được cấu hình là ntp.vngdc.local", "cmd": "cat /etc/systemd/timesyncd.conf", "row_idx": 51},
            {"stt": "42", "muc": "NTP đang đồng bộ thành công", "cmd": "timedatectl timesync-status", "row_idx": 52},
            {"stt": "43", "muc": "Timezone được cấu hình là Asia/Ho_Chi_Minh", "cmd": "timedatectl | grep zone", "row_idx": 53},
        ]},
        {"danh_muc": "8. Logging & Monitoring", "items": [
            {"stt": "44", "muc": "rsyslog được cấu hình để forward log về Syslog Server", "cmd": "grep syslog.vngdc.local /etc/rsyslog.conf", "row_idx": 55},
            {"stt": "45", "muc": "Thư mục lưu log của máy chủ tồn tại trên Syslog Server", "cmd": "Kiểm tra trên syslog server 10.31.100.20", "row_idx": 56},
            {"stt": "46", "muc": "Wazuh agent đang hoạt động", "cmd": "systemctl status wazuh-agent", "row_idx": 57},
            {"stt": "47", "muc": "node_exporter đang hoạt động và phản hồi /metrics", "cmd": "curl http://localhost:9100/metrics | head -5", "row_idx": 58},
            {"stt": "48", "muc": "Target hiển thị trạng thái Healthy/UP trên Prometheus", "cmd": "Kiểm tra trên Prometheus UI", "row_idx": 59},
        ]},
        {"danh_muc": "9. Dịch vụ không cần thiết", "items": [
            {"stt": "49", "muc": "ModemManager disabled", "cmd": "systemctl is-enabled ModemManager", "row_idx": 61},
            {"stt": "50", "muc": "thermald disabled", "cmd": "systemctl is-enabled thermald", "row_idx": 62},
            {"stt": "51", "muc": "apport disabled", "cmd": "systemctl is-enabled apport", "row_idx": 63},
        ]},
        {"danh_muc": "10. Crontab va Scripts", "items": [
            {"stt": "52", "muc": "Script backup_iptables.sh tồn tại", "cmd": "ls -la /opt/dc_script/backup_iptables.sh", "row_idx": 65},
            {"stt": "53", "muc": "Script free_mem.sh tồn tại", "cmd": "ls -la /opt/dc_script/free_mem.sh", "row_idx": 66},
            {"stt": "54", "muc": "Crontab entries có (netfilter-persist, free_mem, clear audit)", "cmd": "sudo cat /etc/crontab", "row_idx": 67},
            {"stt": "55", "muc": "Iptables.save tồn tại và áp dụng các rule", "cmd": "sudo iptables.save", "row_idx": 68},
        ]},
        {"danh_muc": "11. Sudo Hardening", "items": [
            {"stt": "56", "muc": "Defaults use_pty", "cmd": "sudo grep 'use_pty' /etc/sudoers /etc/sudoers.d/ 2>/dev/null", "row_idx": 70},
            {"stt": "57", "muc": "Defaults logfile=/var/log/sudo.log", "cmd": "sudo grep 'logfile' /etc/sudoers /etc/sudoers.d/ 2>/dev/null", "row_idx": 71},
            {"stt": "58", "muc": "Không có NOPASSWD trong sudoers", "cmd": "sudo grep -rP 'NOPASSWD' /etc/sudoers /etc/sudoers.d/ 2>/dev/null | wc -l", "row_idx": 72},
        ]},
        {"danh_muc": "12. File Permissions", "items": [
            {"stt": "59", "muc": "/etc/shadow và /etc/gshadow: permissions = 640, owner = root:shadow", "cmd": "stat -c '%n %a %U:%G' /etc/shadow /etc/gshadow", "row_idx": 74},
            {"stt": "60", "muc": "/etc/passwd và /etc/group: permissions = 644, owner = root:root", "cmd": "stat -c '%n %a %U:%G' /etc/passwd /etc/group", "row_idx": 75},
            {"stt": "61", "muc": "/etc/security/opasswd: permissions = 600", "cmd": "stat -c '%n %a %U:%G' /etc/security/opasswd 2>/dev/null || echo 'file not exist'", "row_idx": 76},
        ]},
    ],
