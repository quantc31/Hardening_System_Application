...
CHECKLIST_DATA = {
    "Linux Ubuntu 24.04": [
        ...
        {"danh_muc": "2. SSH Hardening", "items": [
            ...
            {"stt": "17", "muc": "PermitEmptyPasswords = no", "cmd": "grep PermitEmptyPasswords /etc/ssh/sshd_config", "row_idx": 21},
            {"stt": "18", "muc": "UsePAM = yes", "cmd": "grep 'UsePAM' /etc/ssh/sshd_config", "row_idx": 22},
            {"stt": "19", "muc": "GSSAPIAuthentication = no", "cmd": "grep GSSAPIAuthentication /etc/ssh/sshd_config", "row_idx": 23},
            {"stt": "20", "muc": "HostbasedAuthentication = no / IgnoreRhosts = yes", "cmd": "grep -E 'HostbasedAuthentication|IgnoreRhosts' /etc/ssh/sshd_config", "row_idx": 24},
            {"stt": "21", "muc": "LogLevel = VERBOSE", "cmd": "grep LogLevel /etc/ssh/sshd_config", "row_idx": 25},
            {"stt": "22", "muc": "MaxStartups = 10:30:60", "cmd": "grep MaxStartups /etc/ssh/sshd_config", "row_idx": 26},
            {"stt": "23", "muc": "ClientAliveInterval = 900", "cmd": "grep -E 'ClientAliveInterval|ClientAliveCountMax' /etc/ssh/sshd_config", "row_idx": 27},
            {"stt": "16", "muc": "Cấu hình SSH đã được kiểm tra cú pháp (syntax check)", "cmd": "sudo sshd -t", "row_idx": 28},
        ...
        {"danh_muc": "5. Firewall (Iptables)", "items": [
            ...
            {"stt": "36", "muc": "INPUT default policy là DROP/REJECT", "cmd": "sudo iptables -L INPUT -n | head -5", "row_idx": 44},
            {"stt": "37", "muc": "FORWARD default DROP", "cmd": "sudo iptables -L FORWARD -n | head -3", "row_idx": 45},
